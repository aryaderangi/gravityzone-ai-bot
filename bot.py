...
async def dispatch_action(message: Message, action: str):
    ...
    elif action == "voice_tools":
        await voice_tools(message)
...
async def handle_voice(message: Message):
    ...
    try:
        ...
        if user.get("mode") == "stt":
            update_user(uid, mode="chat")
            await status.edit_text(f"📝 متن شناسایی شده:\n\n{text}")
            return
...
async def voice_tts_info(cb: CallbackQuery):
    ...
    await cb.answer("🔊 حالا متن خود را ارسال کن.", show_alert=True)


@dp.callback_query(F.data == "voice_pick")
